...
"""
sub_dict = {}
with open("lesson_6.txt", encoding='utf-8') as file_obj:
    for line in file_obj:
        cl_line = line.replace("(л)", "").replace("(пр)", "").replace("(лаб)", "").replace("(.)", "")
        subject, lecture, practice, lab = cl_line.split()
        lect_int = 0
        practice_int = 0
        lab_int = 0
        if lecture != "-":
        ...
        if lab != "-":
            lab_int = int(lab)
        sub_dict[subject] = lect_int + practice_int + lab_int
    print(f"{sub_dict}")
    file_obj.close()
